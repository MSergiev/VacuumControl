...
SFE_BMP180::SFE_BMP180( int32_t scl, int32_t sda )
: wire_impl( scl, sda )
// Base library type
{}
...
// Initialize library for subsequent pressure measurements
{
	...
	// Start up the Arduino's "wire_impl" (I2C) library:
	// wire_impl.begin();
	wire_impl.begin();
...
wire_impl.beginTransmission(0);
	wire_impl.beginTransmission(BMP180_ADDR);
	wire_impl.write(values[0]);
	_error = wire_impl.endTransmission();
	if (_error == 0)
	{
		wire_impl.requestFrom(BMP180_ADDR,length);
		while(wire_impl.available() != length) ; // wait until bytes are ready
		for(x=0;x<length;x++)
		{
			values[(int8_t)x] = wire_impl.read();
		}
		return(1);
	}
	return(0);
...
char SFE_BMP180::getError(void)
	...
	// error code using this command. Errors are from the wire_impl library:
	// 0 = Success
